...
function previewStatus(): StaffStatus {
  return {
    store: { code: "CVC01", name: "Cafe Value" }, dayKey: "2026-07-23", submissions: [],
...
async function redrawImage(file: File) {
  ...
  return new File([blob], "cvc-staff-photo.jpg", { type: "image/jpeg", lastModified: Date.now() });
}
  ...
  if (!authenticated) return (
    <main className={styles.shell}>
      <section className={styles.loginCard} aria-labelledby="login-title">
        ...
        <p className={styles.eyebrow}>CVC01 · Staff only</p>
        <h1 id="login-title">Today’s store task</h1>
        <p className={styles.intro}>Enter the Cafe Value staff PIN.</p>
  ...
  return (
    <main className={styles.shell}>
      <header className={styles.header}>
        <div><p className={styles.eyebrow}>CVC01 · Staff</p><h1>Today’s store task</h1><p>{status?.dayKey}</p></div>
