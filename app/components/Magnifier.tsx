...
export default function Magnifier({ src, alt, className }: MagnifierProps) {
  ...
  const [imageSource, setImageSource] = useState({ prop: src, current: src });
  const currentSrc = imageSource.prop === src ? imageSource.current : src;

  const handleImageError = () => {
    if (currentSrc && (currentSrc.indexOf('r2.dev') !== -1 || currentSrc.indexOf('images.torontodispensaryhub.com') !== -1)) {
      const filename = currentSrc.split('/').pop();
      setImageSource({ prop: src, current: `https://athena-cannabis-images.vercel.app/products/${filename}` });
    }
  };
