...
export default function TV2Page() {
  ...
  useEffect(() => {
    const initial = window.requestAnimationFrame(() => { void loadData(); fitToScreen(); });
    window.addEventListener("resize", fitToScreen);
    const refresh = setInterval(loadData, 5*60*1000);
    return () => { window.cancelAnimationFrame(initial); window.removeEventListener("resize", fitToScreen); clearInterval(refresh); };
