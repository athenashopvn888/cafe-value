...
export default function CafeValueWebChat() {
  ...
  useEffect(() => {
    const initial = window.setTimeout(() => {
      if (token) void refresh(token).catch(() => setNotice("Your previous Cafe Value chat could not be restored."));
  ...
  async function start(event: FormEvent) {
    ...
    try {
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/session`, {
        ...
        body: JSON.stringify({ storeId: "CVC01", customerName: intent === "NEW_CUSTOMER" ? name : "", phone, intent, message: firstMessage, workflowVersion: "READY_V1", smsConsent }),
      }));
  ...
  return (
    <aside className={`sod-web-chat ${open ? "open" : ""}`} aria-label="Cafe Value Web Chat">
      ...
      {open && <section className="sod-chat-panel" role="dialog" aria-modal="true" aria-label="Cafe Value Web Chat">
      <header><div><strong>Cafe Value Web Chat</strong><small>Start your delivery order with a dispatcher</small></div><button type="button" onClick={() => setOpen(false)} aria-label="Minimize chat">×</button></header>
