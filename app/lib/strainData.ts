...
export function getStrainData(
  ...
  const description = `${name} is listed as ${tierDesc} in the ${typeLabel} lane at Cafe Value${thcLine}. Compare the current product page, menu notes, weight, and posted price before choosing.`;

  const metaDescription = `${name} at Cafe Value in Toronto. Compare the current ${tierDesc} listing, listing details, and staff guidance before visiting.`;
